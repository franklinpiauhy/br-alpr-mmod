...
SAVE_DIR = 'D:\\UFPR-ALPR dataset\\results_test\\'
...
for f in glob.glob(os.path.join(DIR,'**','*.png'), recursive=True):
    ...
    for k, det in enumerate(dets):
        ...
        cv2.rectangle(clr_img,(det.left(),det.top()),(det.right(),det.bottom()),(0,255,0),2)
        ...
        if r==7.0/7.0:
            all_correct += 1
        cv2.putText(clr_img, plate_text, (det.left(), det.top()), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
    #cv2.imshow('plate', cv2.resize(clr_img,None, fx=0.5, fy=0.5))
    #if cv2.waitKey(1) & 0xFF == ord('q'):
    #    break
    name = os.path.join(SAVE_DIR,f.split('\\')[-1])
    cv2.imwrite(name, clr_img)
